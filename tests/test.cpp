...
TEST_CASE("Sort on disk")
{
    ...
    SECTION("Lazy Sort Manager uniform sort")
    {
        ...
        SortManager manager(memory_len, 16, 4, size, 0, 1);
    ...
    SECTION("Sort in Memory")
    {
        ...
        for (uint32_t i = 0; i < iters; i++) {
            ...
            picosha2::hash256(hash_input.begin(), hash_input.end(), hash.begin(), hash.end());
            hash[0] = hash[1] = 0;
            buffer.insert(buffer.end(), hash.data(), hash.data() + size);
        ...
        UniformSort::SortToMemory(&buffer[0], memory.get(), size, iters, 16);
...
TEST_CASE("DiskProver")
{
    ...
    {
        Plotter plotter = Plotter();
        auto plot = plotter.CreatePlot(18, plot_id_1, 11, 0, 4000);
        Prover prover1(plot);
...
TEST_CASE("FilteredDisk")
{
    ...
    SECTION("filter even")
    {
        ...
        FilteredDisk fd(&d, std::move(filter), 4);
